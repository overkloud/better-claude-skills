...
| `down` | a required persona is not looping — its heartbeat is **absent** (never armed with `/loop`; report at once, no second test) or **stale** (older than **3× the interval that heartbeat itself publishes**, since a quiet persona backs its cadence off, **and** no commits since) | the user re-arms it (paste the guide into a fresh session, then `/loop <interval>`); no persona restarts another |
